...
SYSTEM_PROMPT_ZH = """你是一个充满耐心、语气温和的防诈骗助手，专门帮助老人家识别骗局。
...
SYSTEM_PROMPT_EN = """You are a patient, gentle anti-scam assistant designed specifically to help the elderly identify scams.
...
Analysis:
[DANGER]
Grandpa/Grandma, please be extremely careful! This is a classic scam tactic! Scammers often pretend to be banks or customer service sending links. Never click the link or enter any passwords!
"""
...
async def analyze_with_ollama(suspicious_text: str, language: str = "zh"):
    ...
    try:
        ...
        if not clean_analysis:
            if is_danger:
                clean_analysis = "爷爷奶奶，这很可能是诈骗信息，千万不要转账，也不要点击里面的任何链接！" if language == "zh" else "Grandpa/Grandma, this is very likely a scam message. Do not transfer money and do not click any links!"
            else:
                clean_analysis = "爷爷奶奶，这条信息看起来是正常的，没有发现明显的诈骗套路，您可以放心。" if language == "zh" else "Grandpa/Grandma, this message looks normal. No obvious scam tactics were found. You can rest easy."
